...
ssh -o "StrictHostKeyChecking=no" "$SSH_USER@$SSH_URL" 'docker compose down; docker network prune --force; docker system prune -a --force; docker network create web; docker network create --internal internal; docker rmi $(docker images -aq) --force; docker compose up -d'